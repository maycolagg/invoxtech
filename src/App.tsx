  ...
  useEffect(() => {
    ...
    fetch('/api/shops', {
      ...
      .then(data => {
        ...
        setShops(data);
        if (shopParam) {
          const shop = data.find((s: any) => s.id === parseInt(shopParam) || s.slug === shopParam);
          if (shop) {
    ...
    try {
      ...
      if (res.ok) {
        if (authMode === 'forgot') {
          ...
          setUser(result);
          localStorage.setItem('invox_user', JSON.stringify(result));
          ...
          <div className="hidden lg:flex items-center gap-6">
            ...
            {user ? (
              <div className="flex items-center gap-4">
                ...
                <button 
                  onClick={() => { setUser(null); localStorage.removeItem('invox_user'); setView('landing'); }}
                  ...
                  <>
                    ...
                    <div className="pt-4 border-t border-zinc-100 dark:border-white/5">
                      <div className="flex items-center justify-between px-4">
                        ...
                        <button 
                          onClick={() => { setUser(null); localStorage.removeItem('invox_user'); setView('landing'); setIsMenuOpen(false); }}
      ...
      <main className="pt-20">
        ...
        {view === 'profile' && user && (
          <div className="max-w-4xl mx-auto px-6 py-12">
            <div className={cn(
              "p-10 rounded-[40px] border shadow-2xl space-y-12 transition-all duration-500",
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    <h2 className="text-3xl font-black tracking-tight">Meu Perfil</h2>
                    <p className="text-zinc-500">Gerencie suas informações e histórico</p>
                  </div>
                </div>
                <button 
                  onClick={() => setView('landing')}
                  className="p-3 rounded-2xl bg-zinc-100 dark:bg-zinc-800 text-zinc-500 hover:text-emerald-500 transition-all"
                >
                  <X size={24} />
                </button>
              </div>
              ...
              <div className="space-y-6">
                <h3 className="text-2xl font-black tracking-tight">Histórico de Agendamentos</h3>
                <UserBookingHistory userId={user.id} theme={theme} />
              </div>
            </div>
          </div>
        )}
...
function UserBookingHistory({ userId, theme }: { userId: number, theme: 'light' | 'dark' }) {
  ...
  if (loading) return <div className="text-center py-12 text-zinc-500">Carregando histórico...</div>;
  if (bookings.length === 0) return <div className="text-center py-12 text-zinc-500">Nenhum agendamento encontrado.</div>;

  return (
    <div className="space-y-4">
      {bookings.map((booking) => (
        <div key={booking.id} className={cn(
          "p-6 rounded-3xl border flex flex-col md:flex-row justify-between items-start md:items-center gap-6",
          theme === 'dark' ? "bg-[#0a0a0c] border-white/5" : "bg-zinc-50 border-zinc-100"
        )}>
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 rounded-2xl bg-zinc-200 dark:bg-zinc-800 overflow-hidden">
              <img src={booking.shop_image || `https://picsum.photos/seed/${booking.shop_id}/200/200`} alt={booking.shop_name} className="w-full h-full object-cover" />
            </div>
            <div>
              <h4 className="font-bold text-lg">{booking.shop_name}</h4>
              <p className="text-sm text-zinc-500">{booking.service_name}</p>
              <p className="text-xs text-zinc-400 mt-1">{booking.booking_date} às {booking.start_time}</p>
            </div>
          </div>
      ...
      {showReviewModal && (
        <ReviewModal 
          ...
          userId={userId} 
          shopId={bookings.find(b => b.id === showReviewModal)?.shop_id}
          onClose={() => setShowReviewModal(null)}
          onSuccess={() => {
            setShowReviewModal(null);
            // Refresh bookings
            fetch(`/api/bookings/user/${userId}`, { headers: { 'x-app-integrity': 'invox-core-v1' } })
              .then(res => res.json())
              .then(setBookings);
          }}
          theme={theme}
...
function ReviewModal({ bookingId, userId, shopId, onClose, onSuccess, theme }: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch('/api/reviews', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ booking_id: bookingId, user_id: userId, shop_id: shopId, rating, comment })
    });
    if (res.ok) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/60 backdrop-blur-sm">
      <motion.div 
        ...
        className={cn(
          "w-full max-w-md p-10 rounded-[40px] shadow-2xl space-y-8",
          theme === 'dark' ? "bg-[#141417]" : "bg-white"
        )}
      >
        <div className="text-center space-y-2">
          <h3 className="text-3xl font-black tracking-tight">Avaliar Serviço</h3>
          <p className="text-zinc-500">Como foi sua experiência?</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="flex justify-center gap-2">
            {[1, 2, 3, 4, 5].map((star) => (
              <button
                ...
                className={cn(
                  "p-2 transition-all",
                  rating >= star ? "text-amber-500 scale-110" : "text-zinc-300"
                )}
              >
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold text-zinc-400 uppercase tracking-widest">Comentário (opcional)</label>
            <textarea 
              value={comment}
              onChange={(e) => setComment(e.target.value)}
              className={cn(
                "w-full px-6 py-4 rounded-2xl border-none outline-none transition-all h-32 resize-none",
                theme === 'dark' ? "bg-[#0a0a0c] text-white focus:ring-2 focus:ring-emerald-500" : "bg-zinc-100 text-zinc-900 focus:ring-2 focus:ring-zinc-900"
              )}
          ...
          <div className="flex gap-4">
            <button 
              type="button"
              onClick={onClose}
              className="flex-1 py-4 rounded-2xl font-bold text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-all"
            >
              Cancelar
            </button>
            <button 
              type="submit"
              disabled={loading}
              className="flex-1 py-4 rounded-2xl bg-emerald-600 text-white font-black shadow-xl shadow-emerald-900/20 hover:bg-emerald-500 transition-all disabled:opacity-50"
            >
